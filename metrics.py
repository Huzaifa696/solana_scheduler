...
numbers = re.findall(
    r'scheduler loop time (\d+), count (\d+), packet_reception (\d+), working_bank (\d+), retry_and_garbage_collection (\d+), batch_processing (\d+), buffering_time (\d+), un_buffering_time (\d+), tx_counter (\d+), tx_recv_counter (\d+), not_leader_counter (\d+), vote_counter (\d+), empty_buffer (\d+), buffered_packets (\d+), retry_counter (\d+)', log_contents)
